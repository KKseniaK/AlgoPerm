...
static class Algo
{
    ...
    public static List<List<string>> BuildGanttByPriorityScan(Graph g, int processors)
    {
...
static class GraphViz
{
    public static void SaveDot(Graph g, string path)
    {
        ...
        {
            string color;
            if (v.Children.Count == 0) color = "palegreen";
            else if (v.Parents.Count == 0) color = "lightblue";
            else color = "white";

            lines.Add($"  {v.Name} [fillcolor=\"{color}\", label=\"{v.Name}\\nX={v.PriorityLex}\"];");
        }
...
static class Input
{
    public static (List<char> pred, List<char> succ) GetDemoManual()
    {
    ...
    public static (List<char> pred, List<char> succ) GetDemoSeminar()
    {
        var pred = new List<char> { 'A', 'A', 'C', 'C', 'D', 'E', 'F', 'G', 'G', 'H', 'H', 'I', 'I', 'L' };
        var succ = new List<char> { 'B', 'J', 'E', 'F', 'E', 'H', 'H', 'J', 'K', 'A', 'G', 'H', 'L', 'G' };
        return (pred, succ);
    }

    public static (List<char> pred, List<char> succ) ReadFromKeyboard()
    {
        Console.WriteLine("Введите предков (через пробел или запятую), пример: A A B C");
        var p = ParseLetters(Console.ReadLine());

        Console.WriteLine("Введите потомков (той же длины), пример: G H H M");
        var s = ParseLetters(Console.ReadLine());

        if (p.Count != s.Count)
            throw new ArgumentException("Количество предков и потомков должно совпадать.");

        return (p, s);
    }

    static List<char> ParseLetters(string line)
    {
        ...
        foreach (var part in parts)
        {
            ...
            if (c < 'A' || c > 'Z')
                throw new ArgumentException($"Некорректный символ: {t}");
            res.Add(c);
        }
...
class Program
{
    static void Main()
    {
        Console.WriteLine("LEXICOGRAPHIC STRATEGY");
        Console.WriteLine("1) Демо (решено руками)");
        Console.WriteLine("2) Демо (семинар)");
        Console.WriteLine("3) Ввод с клавиатуры");
        Console.Write("Выбор: ");

        var choiceLine = Console.ReadLine();
        int choice = int.TryParse(choiceLine, out var c) ? c : 1;

        List<char> pred, succ;

        if (choice == 2)
            (pred, succ) = Input.GetDemoSeminar();
        else if (choice == 3)
            (pred, succ) = Input.ReadFromKeyboard();
        else
            (pred, succ) = Input.GetDemoManual();
        ...
        var gantt = Algo.BuildGanttByPriorityScan(g, 2);
        Print.Gantt(gantt, "GANTT: LEX STRATEGY (priority scan, 2 processors, unit tasks)");
